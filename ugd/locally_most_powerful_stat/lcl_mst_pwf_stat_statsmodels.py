...
This function computes joint maximum likelihood estimates of the network edge probabilitys under the H0 model.
These edge probabilities are used to construct an estimate of the local best test statistic in a specific
direction.
...
Procedure:
    ...
    2) estimate the logit model

    3) compute fitted edge probabilities

    4) reshape into the matrix form of edge probabilities
...
def make_lcl_mst_pwf_stat(adj_m, var_dict=None, controls=None, edge_util_function = None):
    ...
    # 1) parse the graph and node_dict into the matrix input shape for the logit model
    x,y, edges_removed_due_to_sparsity = create_logit_variables(adj_m, var_dict, controls)
    ...
    logging.info("Maximum gradient entry at the MLE: "+ str(np.max(np.abs(logit_mod.score(logit_res.params)))))
    hessian = - logit_mod.hessian(logit_res.params)
    eigenvalues = np.linalg.eigvalsh(hessian)
    logging.info("Eigenvalues of Hessian matrix:     ")
    ...
    optimal_stat = create_lcl_mst_pwf_statistic_from_thresholds(threholds_m, n, edge_util_function = edge_util_function)
    return optimal_stat




def create_logit_variables(adj_m, var_dict, controls):
    "bring the matrix shaped node variables into a table form so that a logit package can be used"
    ...
    if not(var_dict is None or controls is None):
        ...
        sort_partition(node_set_partition )
    ...
    because of mulitcoliniarity. By adjusting the out and in affinity of all edges in one group
    so that within one group the affinity is the same and to the next node group edges are formed 
    more likely. (the backward edges form the other group can be corrected for with the group parameter
    from the other direction) 
    
    Therefore we correct for this fact by for some partition adjancy matrix edges have
    only coefficients in one direction, such that these partition adjacency matrix edges span
    a tree over the partition graph ( therefore len(node_set_partition)-1 )
    
    due to the colums removed because of sparsity, we don't know whether the last len(node_set_partition)-1 group 
    ...
    if np.linalg.matrix_rank(x) < x.shape[1]:
        print("ATTENTION: the null space of x is not 0, there is multicolinearity")
...
def remove_sparse_edges(x,y):
    """
        ...
        or there are no edges from one nodegroup to another.
        In this case we do not consider these "edges" because the coefficients of the node (or nodepartition)
        would optimize the likelihood at infinity.

        We therefore exclude the edge and the parameter from the estimation.
        For the optimal test staistic the thresholds have to be set for each potential edge.
        The threshholds of these potential edges are set to minus infinity, so that they don't contribute
        to the test statistic. All simulated graphs will not have an edge realisation at these potential edges
    ...
    logging.info("A total of " + str(sparse_edges.__len__()) + " directed dyads are excluded from estimation because they include a sparse "
                                                               "node (one with no arcs directed in or out or due to a "
...
def sort_partition(node_set_partition):
     ...
     multicolinearity.  This function induces a fixed order (at least in the cases where the elements sum is unique for
     each element)"""
